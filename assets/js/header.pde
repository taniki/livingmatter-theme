...
class Seed{
  ...
  void plant(Seed parent){
    ...
    new_joint.x = (int) this.x;
    new_joint.y = (int) this.y;
...
void load_SUN(){
  ...
  Distractor[] ds = {
    new Distractor(0,0, 120)
  };

  for(int i = 0; i < 8; i++){
    Distractor d = new Distractor( int(120*cos(i*PI/4)), int(120*sin(i*PI/4)), 30);
    ds = (Distractor[]) append(ds, d);
  }
...
void load_LM(){
  ...
  for(int i = 0; i < 10; i++){
    ...
    Distractor d = new Distractor(x,0,radius);
    ds = (Distractor[]) append(ds, d);
  }
...
//        play_sound = true;
  ...
  if(joints.size() < density){
    if(seeds.size() <= inial_seeds){
      for(int i = inial_seeds - seeds.size(); i >= 0; i--){
        Seed s = new Seed();
        s.reboot();
        seeds.add(s);
      }
